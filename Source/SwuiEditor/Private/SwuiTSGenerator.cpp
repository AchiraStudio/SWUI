...
static FString GetSwuiClientPackageName()
{
	const FString PkgJsonPath = FPaths::ProjectContentDir() / TEXT("UI/package.json");
	FString PkgJsonContent;
	if (FFileHelper::LoadFileToString(PkgJsonContent, *PkgJsonPath) && PkgJsonContent.Contains(TEXT("@swui/core")))
	{
		return TEXT("@swui/core");
	}
	return TEXT("@simplewebui/client");
}
	...
	const FString& ContractBlock)
{
	...
	Vars.Add(TEXT("ClientPackage"), GetSwuiClientPackageName());
...
import Swui from '${ClientPackage}';
...
bool FSwuiTSGenerator::GenerateNavigation(USwui* Bridge, const TArray<FSwuiNavigationEvent>& NavigationEvents)
{
	...
	const FString Header = FString::Printf(
		...
		TEXT("import Swui from '%s';\n\n"),
		*InterfaceName,
		*GetSwuiClientPackageName());
