...
// ---------------------------------------------------------------------------
// UV and raw-pixel input forwarding (world-space surfaces)
// ---------------------------------------------------------------------------

bool USwuiView::UVToBrowserPixel(const FVector2D& UV, int32& OutX, int32& OutY) const
{
	if (Width <= 0 || Height <= 0)
	{
		return false;
	...
	OutX = FMath::Clamp(FMath::RoundToInt(UV.X * static_cast<float>(Width)),  0, Width  - 1);
	OutY = FMath::Clamp(FMath::RoundToInt(UV.Y * static_cast<float>(Height)), 0, Height - 1);
	return true;
}

bool USwuiView::ForwardMouseMoveAtPixel(int32 BX, int32 BY)
{
	if (!bPointerInputEnabled)
	{
		return false;
...
bool USwuiView::ForwardMouseButtonAtPixel(int32 BX, int32 BY, FKey Button, bool bMouseUp, int32 ClickCount)
{
	if (!bPointerInputEnabled)
	{
		return false;
...
bool USwuiView::ForwardMouseWheelAtPixel(int32 BX, int32 BY, float DeltaX, float DeltaY)
{
	if (!bPointerInputEnabled)
	{
		return false;
