...
public:
	...
	// Use this for world-space surfaces where no Slate viewport geometry is available.
	bool UVToBrowserPixel(const FVector2D& UV, int32& OutX, int32& OutY) const;
	...
	// ---- Raw browser-pixel input forwarding (no Slate viewport lookup) ----
	// Use these from world-space surfaces: pass already-resolved browser pixel
	// coordinates obtained via UVToBrowserPixel().

	bool ForwardMouseMoveAtPixel(int32 BX, int32 BY);

	bool ForwardMouseButtonAtPixel(
		int32 BX,
		int32 BY,
		FKey Button,
		bool bMouseUp,
